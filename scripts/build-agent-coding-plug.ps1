...
$author = Join-Path $repo "reference-plugs\tethers-agent-coding\author"
$buildProvider = Join-Path $repo "tethers-0.1\host-rust\target\release\agent_coding_provider.exe"
$buildHost = Join-Path $repo "tethers-0.1\host-rust\target\release\tethers.exe"
if ([string]::IsNullOrWhiteSpace($Output)) { $Output = Join-Path $repo "tethers-0.1\dist\tethers-agent-coding-0.1.0.tetherplug" }
$Output = [System.IO.Path]::GetFullPath($Output)
$outputParent = Split-Path -Parent $Output
New-Item -ItemType Directory -Force -Path $outputParent | Out-Null
$stage = Join-Path ([System.IO.Path]::GetTempPath()) ("tethers-agent-coding-pack-" + [guid]::NewGuid().ToString("N"))
try {
    Write-Host "Building the reviewed coding provider..."
    & cargo build --manifest-path $hostManifest --release --locked --bin agent_coding_provider --bin tethers
    ...
    Copy-Item -LiteralPath $buildProvider -Destination (Join-Path $stage "provider\agent_coding_provider.exe")
