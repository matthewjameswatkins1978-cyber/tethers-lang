#!/usr/bin/env bash
# Clean-package smoke for a Tethers Linux runtime (0.8 authority architecture).
#
...
# tree, Cargo target, or developer-home dependency. PATH is sanitised to the
# system set so binaries cannot leak in from elsewhere.
set -Eeuo pipefail
...
Run smoke tests on an extracted Linux Tethers package directory.
EOF
}
...
# Sanitise PATH: system directories only, no Cargo target, no checkout bins.
...
printf '=== TETHERS LINUX PACKAGE SMOKE ===\n'
printf 'Package: %s\n\n' "$package_dir"
...
printf '\n=== ALL 8 PACKAGE SMOKE TESTS PASSED ===\n'
